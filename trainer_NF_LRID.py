...
class NF_LRID_Trainer(Base_Trainer):
    def __init__(self):
        parser = NF_LRID_Parser()
    ...
    def preprocess(self, data, mode='train', preprocess=True):
        ...
        if self.use_gpu and mode=='train' and preprocess:
            b = imgs_lr.shape[0]
            if self.args['dst_train']['dataset'] == 'IMX686_Raw_Dataset':
...
class NF_LRID_Parser():
    def __init__(self):
        self.parser = argparse.ArgumentParser(formatter_class=argparse.ArgumentDefaultsHelpFormatter)

    def parse(self):
        self.parser.add_argument('--runfile', '-f', default="runfiles/IMX686/NoiseFlow.yml", type=Path, help="path to config")
