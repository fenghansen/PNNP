...
class IMX686_Trainer(Base_Trainer):
    def __init__(self):
        ...
        self.legal_ratio = torch.tensor([1, 2, 4, 8, 16]).float().to(self.device)
        if 'arch_proxy' in self.args and self.mode=='train':
            ...
            model_path = os.path.join(f'{self.fast_ckpt}/IMX686_NoiseFlow_last_model.pth')
            model = torch.load(model_path, map_location=self.device)
    ...
    def preprocess(self, data, mode='train', preprocess=True):
        ...
        if self.use_gpu and mode=='train' and preprocess:
            ...
            if self.args['dst_train']['dataset'] == 'IMX686_Mix_Dataset':
                ...
                data['ratio'] = self.legal_ratio[np.random.randint(len(self.legal_ratio))]
                kwargs = {
                    'clean': imgs_hr / data['ratio'],
                    'iso': data['ISO'][0].float().to(self.device),
                }
                with torch.no_grad():
