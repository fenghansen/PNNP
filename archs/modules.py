...
class ResBlock_Dist(nn.Module):
    def __init__(self, nf=8, k=1, act='swish'):
        super().__init__()
        self.conv1 = Conv1x1(nf, k, act)
        self.conv2 = Conv1x1(nf, k, act)
...
class Conv1x1(nn.Module):
    def __init__(self, nf=16, k=1, act='swish'):
        super().__init__()
        self.conv = nn.Conv2d(nf, nf, k, 1, padding='same')
        if act=='swish':
            self.act = nn.SiLU()
        elif act=='lrelu':
