...
class RealBase_Dataset(Dataset):
        ...
        for i in range(self.length):
            ...
            if 'darkshading' in self.args['command']:
                ...
                if 'darkshading2' in self.args['command'] and iso not in self.noiseparam:
                    self.noiseparam[iso] = get_specific_noise_params(self.args['camera_type'], iso)
    ...
    def record_bias_frames(self):
        ...
        try:
            ...
            self.legalISO = np.array([50, 64, 80, 100, 125, 160, 200, 250, 320, 400, 500, 640, 800, 1000, 1250, 1600, 2000, 2500, 3200, 4000, 5000, 6400, 8000, 10000, 12800, 16000, 20000, 25600])
            log(str(e))
...
class SID_Dataset(RealBase_Dataset):
        ...
        if self.args['mode'] == 'train':
            ...
            if 'cache' in self.args['command'].lower():
                ...
                for idx in tqdm(range(self.length)):
                    self.buffer.append(np.array(dataload(self.infos[idx]['long'])).reshape(self.H,self.W))
        else:
            self.evaltest_remap()
    ...
    def __getitem__(self, idx):
        ...
        hr_raw = np.array(dataload(self.infos[idx]['long'])).reshape(self.H,self.W) if len(self.buffer) == 0 else self.buffer[idx]
...
class PMNNP_Dataset(SID_Dataset):
    def __init__(self, args=None):
        super().__init__(args)
        self.initialization()
    ...
    def __getitem__(self, idx):
        ...
        if 'darkshading' in self.args['command']:
            lr_raw = lr_raw - self.get_darkshading(iso=data['ISO'], exp=data['exp'], naive=self.naive)
            # if self.hot_check(data['name']) and not data['black_lr']: lr_raw+=2
            if 'darkshading2' in self.args['command'] and self.args["mode"] == 'train':
                # SID配对数据训练的时候都要减长曝光的BLE（相机长曝光自动矫正FPN了）
                # BLE = self.get_BLE_long(data['ISO'], data['ExposureTime'], self.naive) + self.blc_mean['kt_b_long'][data['ISO']]
                # hr_raw = hr_raw - BLE
                lr_raw += np.random.randn() * self.noiseparam[data['ISO']]['biassig']
        elif 'blc' in self.args['command']:
            # 只使用均值矫正
...
class TestDataset(RealBase_Dataset):
    ...
    def __getitem__(self, idx):
        ...
        data['ISO'] = self.isos[idx]
        if self.args['clip']:
            lb = -np.inf if self.args['clip']==HALF_CLIP else 0
            data['data'] = data['data'].clip(lb,1)
