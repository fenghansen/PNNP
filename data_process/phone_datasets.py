...
class PhoneBase_Dataset(Dataset):
    ...
    def initialization(self):
        ...
        if 'cache' in self.args['command'].lower():
            ...
            for idx in tqdm(range(len(self.infos))):
                self.cache_gt.append(np.array(dataload(self.infos[idx]['hr'])).reshape(self.H,self.W))
        # load darkshading
        self.darkshading = {}
        ...
        if iso not in self.noiseparam and self.args['mode'] == 'train':
    ...
    def change_ratio_list(self, ratio_list=[1,2,4,8,16]):
        ...
        self.dgain = ratio_list[-1]#[len(ratio_list)//2]
...
class IMX686_PMNNP_Dataset(PhoneBase_Dataset):
    def __init__(self, args=None):
        super().__init__(args)
        self.initialization()
...
class IMX686_NF_Syn_Dataset(PhoneBase_Dataset):
    def __init__(self, args=None):
        super().__init__(args)
        self.initialization()
...
class IMX686_Proxy_Dataset(PhoneBase_Dataset):
    def __init__(self, args=None):
        super().__init__(args)
        self.initialization()
...
class IMX686_SFRN_Raw_Dataset(PhoneBase_Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.args['mode'] == 'train':
            ...
            lr_crops = black_crops
            lr_shape = lr_crops.shape
            data['ratio'] = np.random.uniform(low=1., high=16., size=lr_shape[0])
            ...
            if self.args['gpu_preprocess'] is False:
                ...
                if 'darkshading2' in self.args['command'] and self.args['mode'] == 'train':
                    # there are no darkshading on phone's GT
                    bias_jitter = np.random.randn() * 0.08
                    black_crops = black_crops + bias_jitter
