...
class PNNP_IMX686_Trainer(Base_Trainer):
    def __init__(self):
        parser = PNNP_LRID_Parser()
    ...
    def preprocess(self, data, mode='train', preprocess=True):
        ...
        if self.use_gpu and mode=='train' and preprocess:
            b = imgs_lr.shape[0]
            if self.args['dst_train']['dataset'] == 'IMX686_Proxy_Dataset':
                ...
                for i in range(b):
                    ...
                    # noise_param['ratio'] = torch.rand(1, device=self.device) * 15 + 1
                    data['ratio'][i] = noise_param['ratio']
                    noise_param['shape'] = (4, 1, d, d)
                    proxy_net = self.PNNP_hot if np.random.randint(2) else self.PNNP_cold
                    imgs_lr[i] = generate_noisy_proxy(proxy_net, y=imgs_lr[i], param=noise_param, 
                                    noise_code=self.dst['noise_code'], ori=self.dst['ori'])
            elif self.args['dst_train']['dataset'] == 'IMX686_NF_Syn_Dataset':
                data['ratio'] = torch.rand(b, dtype=torch.float32, device=self.device) * 15 + 1
                for i in range(b):
                    if 'params' not in self.dst:
                        noise_param = sample_params_proxy(camera_type=self.dst['camera_type'])
                    else:
                    ...
                    proxy_net = self.noiseflow
                    imgs_lr[i] = generate_noisy_flow(proxy_net, y=imgs_lr[i], param=noise_param, 
                                    noise_code=self.dst['noise_code'], ori=self.dst['ori'])
            # BiSNA + PNNP
            elif self.args['dst_train']['dataset'] == 'IMX686_PMNNP_Dataset':
                d = self.dst['patch_size']
    ...
    def load_proxynet(self):
        ...
        # IMX686 实际迁移版使用 cold/hot 两套分段 PNNP。
        # 训练时随机选择一段生成读噪声，以覆盖普通暗帧和热态暗帧的统计差异。
        proxy_name = self.proxy['name']
        self.PNNP_cold = PNNP(self.proxy).requires_grad_(False)
        self.PNNP_cold.ISO2K_require_grad(False, self.proxy['ISO2K'])
        self.PNNP_hot = PNNP(self.proxy).requires_grad_(False)
        self.PNNP_hot.ISO2K_require_grad(False, self.proxy['ISO2K'])
        # Loading Model
        proxy_model_cold = torch.load(f"{self.fast_ckpt}/{proxy_name}_cold.pth", map_location=self.device)
        proxy_model_hot = torch.load(f"{self.fast_ckpt}/{proxy_name}_hot.pth", map_location=self.device)
        self.PNNP_cold = load_weights(self.PNNP_cold, proxy_model_cold, by_name=True).to(self.device)
        self.PNNP_hot = load_weights(self.PNNP_hot, proxy_model_hot, by_name=True).to(self.device)

        ds_k_cold = dataload(f'{self.proxy["ds_dir"]}/darkshading_tk.npy').astype(np.float32) / (6400/10) # ExposureTime
        ds_b_cold = dataload(f'{self.proxy["ds_dir"]}/darkshading_tb.npy').astype(np.float32)
        self.PNNP_cold.ds_k = torch.from_numpy(ds_k_cold).to(self.device)
        self.PNNP_cold.ds_b = torch.from_numpy(ds_b_cold).to(self.device)
        self.PNNP_cold.pack_ds()

        ds_k_hot = dataload(f'{self.proxy["ds_dir"]}/darkshading_tk_hot.npy').astype(np.float32) / (6400/10) # ExposureTime
        ds_b_hot = dataload(f'{self.proxy["ds_dir"]}/darkshading_tb_hot.npy').astype(np.float32)
        self.PNNP_hot.ds_k = torch.from_numpy(ds_k_hot).to(self.device)
        self.PNNP_hot.ds_b = torch.from_numpy(ds_b_hot).to(self.device)
        self.PNNP_hot.pack_ds()
...
class PNNP_LRID_Parser():
    def __init__(self):
        self.parser = argparse.ArgumentParser(formatter_class=argparse.ArgumentDefaultsHelpFormatter)

    def parse(self):
        self.parser.add_argument('--runfile', '-f', default="runfiles/IMX686/PNNP+DSC2.yml", type=Path, help="path to config")
